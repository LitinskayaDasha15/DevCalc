...
public class Calc extends Loan {
	...
	//Вычисление каждого платежа по крдиту
		public List<Payment> CalculateDetailedPayments()
		{
			...
			{
				count++;
				percents = RoundTo(loanRemain*monthRate, 2);
				mainPay = RoundTo((loanRemain + percents >= staticPayment) ? staticPayment - percents : loanRemain, 2);
				loanRemain = RoundTo(loanRemain - mainPay, 2);
				dateOfPayment = dateOfPayment.plusMonths(1);
				
				payments.add(new Payment(count, Integer.toString(dateOfPayment.getYear()) + " " + engRus.get(dateOfPayment.getMonth().toString().toLowerCase()), RoundTo(percents + mainPay, 2), mainPay, percents, loanRemain));
			}
			return payments;
		}
